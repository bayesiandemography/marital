...
sex <- c("Female", "Male")
time <- unique(marriages_divorces_df$time)
...
SM <- crossing(sex, time, status_orig = "Single", status_dest = "Married") %>%
    left_join(filter(marriages_divorces_df, event == "First marriage"), by = c("time" = "time")) %>%
...
MD <- crossing(sex, time, status_orig = "Married", status_dest = "Divorced") %>%
    left_join(filter(marriages_divorces_df, event == "Divorce"), by = c("time" = "time")) %>%
    mutate(count = count / 2) %>% # because splitting between 2 sexes
    select(sex, time, status_orig, status_dest, count)
...
marriages_divorces <- bind_rows(SS, SM, SD, SW,
                                ...
                                DS, DM, DD, DW,
                                WS, WM, WD, WW) %>%
    mutate(status_orig = factor(status_orig, levels = c("Single", "Married", "Divorced", "Widowed")),
           status_dest = factor(status_dest, levels = c("Single", "Married", "Divorced", "Widowed"))) %>%
...
saveRDS(marriages_divorces,
        file = "out/marriages_divorces.rds")
